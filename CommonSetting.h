...
public:
    ...
    static void DateBankUp()
    {
        //新建文件夹名称
        QString FolderName =
                CommonSetting::GetCurrentDateTimeNoSpace();
        //判断目录是否已经存在,如果已经存在,则删除
        QDir targetDir(FolderName);
        if(targetDir.exists()){
            system(tr("rm -rf /mnt/%1")
                   .arg(FolderName).toAscii().data());
        }
        //创建文件夹
        system(tr("mkdir -p /mnt/%1")
               .arg(FolderName).toAscii().data());

        //将/mnt目录的图片数据移动到新建的文件夹中
        system(tr("mv /mnt/*.txt /mnt/%1")
               .arg(FolderName).toAscii().data());
    }
};
